...
def filter_files(path, lda, eps, sigma, N, c, avn0, tol, max_iter, damping, nseq):
    ...
    pattern = f'IBMF_seq_from_input_RRG_Lotka_Volterra_final_av0_{avn0}_T_*_lambda_{lda}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_mu_*_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt'

    # Iterate through the files in the specified directory
    for filename in os.listdir(path):
        if fnmatch.fnmatch(filename, pattern):
            parts = filename.split('_')
            files_T_mu.append((filename, float(parts[11]), float(parts[21])))
...
def print_summary(path_in, path_out, lda, eps, sigma, N, c, avn0, tol, max_iter, ndigits, damping, nseq):
    fout = open(f'{path_out}/IBMF_seq_from_input_RRG_PD_Lotka_Volterra_summary_av0_{avn0}_lambda_{lda}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    ...
    for vals in vals_list:
        ...
        fout.write(f"{int(T * 10 ** ndigits)} {int(mu * 10 ** ndigits)} {av_time:.6f} {av_num_div:.6f} {samples_div_m} {samples_multiple_eq} {samples_with_deaths} {av_m:.6f} {std_av_m:.6f} {nsamples}\n")
    fout.close()
...
def main():
    ...
    path_in = "/media/david/Data/UH/Grupo_de_investigacion/Ecology/Results/IBMF/AllData/from_input/"
